...
def test_put_user_twice_returns_correct_results(base_testing_app: FastAPI, sample_full_user_profile: FullUserProfile) -> None:
    with TestClient(base_testing_app) as testing_app:
        user_id = 1
        response = testing_app.put(f"/user/{user_id}", json=sample_full_user_profile)
...
def test_rate_limit_works(base_testing_app: FastAPI, testing_rate_limit: int, sample_full_user_profile: FullUserProfile) -> None:
    user_id = 1
    with TestClient(base_testing_app) as testing_app:
